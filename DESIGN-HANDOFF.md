# 3b4ff392-2ee2-4140-8b44-6f273c388712 implementation handoff

This archive is the source of truth for turning the design into production code. Start from `opservo-landing.html`, then preserve the visual system, responsive behavior, and interactions found in the exported files.
...
## Source map
- Primary entry: `opservo-landing.html`
...
- A single self-contained `opservo-landing.html` is acceptable only when the export truly contains one user-facing screen and its CSS/JS are structured enough to extract tokens, components, states, and behavior.
...
1. Open `opservo-landing.html` and `DESIGN-MANIFEST.json`; identify every screen file, launcher/overview file, app module, and interaction before coding.
...
1. Inspect `opservo-landing.html` and `DESIGN-MANIFEST.json` first and identify reusable components before coding.
